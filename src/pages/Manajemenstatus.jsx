    ...
    const changeStatus = async (student, toStatus, options = {}) => {
        ...
        try {
            ...
            if (options.toClass) {
                ...
                updateData.class_name = options.toClass
            }
            if (options.toYear) updateData.academic_year = options.toYear
            if (toStatus === 'alumni') {
                updateData.graduation_year = options.graduationYear || student.academic_year
            }
            ...
            await fetchStudents()
            setSuccess(`Status ${student.name} berhasil diubah ke "${STATUS_LABELS[toStatus]?.label}".`)
    ...
    const handleBulkAction = async () => {
        ...
        for (const student of selected) {
            try {
                ...
                if (showBulkModal === 'alumni') {
                    updateData.graduation_year = bulkTargetYear || student.academic_year
                }
        ...
        setBulkNote('')
        await fetchStudents()
        setSuccess(`${successCount} siswa berhasil diproses.`)
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                    Proses naik kelas, kelulusan, dan kelola data alumni siswa SMP IT Al Anshar.
                </p>
            </div>
            ...
            {/* Filter & Search */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row gap-3 items-end">
                ...
                <div className="flex gap-2 flex-wrap">
                    ...
                    >
                        ...
                        {CLASS_ORDER.map(c => <option key={c} value={c}>Kelas {c}</option>)}
                    </select>
                    <select
                        value={filterYear}
                ...
                {selectedIds.size > 0 && (
                    <div className="flex gap-2 flex-wrap">
                        ...
                        {filterStatus === 'aktif' && (
                            <>
                                <button
                                    onClick={() => setShowBulkModal('naik_kelas')}
                                ...
                                <button
                                    onClick={() => setShowBulkModal('lulus')}
                        ...
                        {(filterStatus === 'lulus' || filterStatus === 'naik_kelas') && (
                            <button
                                onClick={() => setShowBulkModal('alumni')}
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-800/60 text-sm">
                            {loading ? (
                                <tr>
                                    <td colSpan={filterStatus === 'alumni' ? 8 : 7} className="py-10 text-center text-slate-500 text-xs animate-pulse">
                                        Memuat data...
                                    </td>
                                </tr>
                                ...
                                filteredStudents.map(student => {
                                    ...
                                    return (
                                        <tr
                                            key={student.id}
                                            className={`transition-all ${isSelected ? 'bg-emerald-500/5' : 'hover:bg-slate-900/30'}`}
                                        >
                                            ...
                                            <td className="py-3 px-4">
                                                ...
                                                {student.previous_class && (
                                                    <p className="text-[10px] text-slate-500 mt-0.5">Sebelumnya: {student.previous_class}</p>
                                                )}
                                            </td>
                                            <td className="py-3 px-4">
                                            ...
                                            {filterStatus === 'alumni' && (
                                                <td className="py-3 px-4 text-slate-400 text-xs">{student.graduation_year || '-'}</td>
                                            )}
                                            <td className="py-3 px-4">
                                                <div className="flex justify-center items-center gap-1.5 flex-wrap">
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 bg-slate-800 border border-slate-700 text-slate-400 hover:text-slate-200 rounded-lg transition-all cursor-pointer"
                                                    ...
                                                    {canNaikKelas && (
                                                        <button
                                                            onClick={() => {
                                                                ...
                                                                if (!nextClass) return
                                                                if (window.confirm(`Naikkan ${student.name} dari ${student.class_name} ke ${nextClass}?`)) {
                                                                    changeStatus(student, 'naik_kelas', {
                                                                        toClass: nextClass,
                                                                        toYear: (() => {
                                                    ...
                                                    {canLulus && (
                                                        <button
                                                            onClick={() => {
                                                                if (window.confirm(`Luluskan ${student.name} dari kelas ${student.class_name}?`)) {
                                                                    changeStatus(student, 'lulus', {
                                                                        graduationYear: student.academic_year,
                                                                        note: 'Lulus SMP IT Al Anshar',
                                                                    })
                                                                }
                                                            }}
                                                            disabled={isProcessing}
                                                    ...
                                                    {canAlumni && (
                                                        <button
                                                            onClick={() => {
                                                                if (window.confirm(`Arsipkan ${student.name} sebagai alumni?`)) {
                                                                    changeStatus(student, 'alumni', {
                                                                        graduationYear: student.graduation_year || student.academic_year,
                                                                    })
                                                                }
                                                            }}
                                                            disabled={isProcessing}
                                                    ...
                                                    {student.status !== 'aktif' && (
                                                        <button
                                                            onClick={() => {
                                                                if (window.confirm(`Kembalikan status ${student.name} ke Aktif?`)) {
                ...
                {filteredStudents.length > 0 && (
                    <div className="px-4 py-3 border-t border-slate-800/60 bg-slate-950/20 flex justify-between items-center">
                        ...
                        {selectedIds.size > 0 && (
                            <span className="text-[11px] text-emerald-400 font-semibold">{selectedIds.size} siswa dipilih</span>
                        )}
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-xl p-6 space-y-4">
                        <div className="flex justify-between items-center pb-3 border-b border-slate-800">
                            ...
                            <button onClick={() => setShowHistory(null)} className="text-slate-500 hover:text-slate-300 cursor-pointer">
                                <X size={16} />
                            </button>
                        </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-xl p-6 space-y-5">
                        <div className="flex justify-between items-center pb-3 border-b border-slate-800">
                            ...
                            <button onClick={() => setShowBulkModal(null)} className="text-slate-500 hover:text-slate-300 cursor-pointer">
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        <div className="space-y-3">
                            {showBulkModal === 'naik_kelas' && (
                                <div>
                                    ...
                                    />
                                    <p className="text-[10px] text-slate-500 mt-1">Biarkan kosong untuk otomatis sesuai kelas masing-masing siswa</p>
                                </div>
                            )}
                            <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                            ...
                            <div>
                                ...
                                <textarea
                                    rows={2}
                                    placeholder="Contoh: Naik kelas reguler T.A. 2026/2027"
                                    value={bulkNote}
                        ...
                        <div className="flex gap-3 justify-end pt-2 border-t border-slate-800">
                            <button
                                ...
                                className="px-4 py-2 text-xs font-bold text-slate-400 bg-slate-950 border border-slate-800 rounded-xl cursor-pointer hover:text-slate-200"
                            >
                                Batal
                            </button>
                            <button
